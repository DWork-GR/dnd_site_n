...
async function api(url, options = {}) {
  ...
  if (!response.ok)
    throw new Error(
      data.error ||
        (response.status === 404
          ? "Сервер ещё не обновлён. Перезапустите npm.cmd start."
...
function renderSpells() {
  $("#player-spells").innerHTML = spells.length
    ? spells
        .map(
          (spell, index) => `
    <article class="spell-card" data-spell="${index}">
      ...
      <label class="spell-description">Описание и эффект
        ...
        .join("")
    : "<p class='empty'>В гримуаре пока нет заклинаний.</p>";
}

function renderInventory() {
  $("#player-inventory").innerHTML = inventoryItems.length
    ? inventoryItems
        .map(
          (item, index) => `
    <article class="inventory-card ${item.magic ? "is-magic" : ""}" data-inventory-item="${index}">
      ...
      <button type="button" class="danger" data-delete-inventory-item="${index}">Удалить предмет</button>
        ...
        .join("")
    : "<p class='empty'>Инвентарь пока пуст. Добавьте первый предмет или артефакт.</p>";
}

function renderFeatures() {
  $("#player-feature-cards").innerHTML = featureCards.length
    ? featureCards
        .map((feature, index) =>
          feature._editing
            ? `
    ...
    <article class="feature-card-view" data-player-feature="${index}">
      ...
      <div>${window.RichText.render(feature.description || "", escapeHtml) || "<span class='feature-empty-description'>Описание не заполнено</span>"}</div>
        ...
        .join("")
    : "<p class='empty'>Добавьте первую способность персонажа.</p>";
  window.RichText.enhance($("#player-feature-cards"));
}

function renderCharacterCards() {
  $("#player-attack-cards").innerHTML =
    attackCards
      .map((item, index) =>
        item._editing
          ? `<article class="feature-card-editor weapon-card-editor" data-player-attack="${index}"><div class="weapon-fields"><input data-field="name" value="${escapeHtml(item.name || "")}" placeholder="Название оружия"><input data-field="damageDie" value="${escapeHtml(item.damageDie || "")}" placeholder="Кость: 1d8"><label><input data-field="proficient" type="checkbox" ${item.proficient ? "checked" : ""}> Владение</label></div><textarea data-field="description" placeholder="Описание оружия">${escapeHtml(item.description || "")}</textarea><div class="card-edit-actions"><button type="button" class="danger" data-delete-player-attack="${index}">Удалить</button><button type="button" class="feature-save-button" data-save-player-attack="${index}">Сохранить оружие</button></div></article>`
          : `<article class="feature-card-view weapon-card-view"><button type="button" class="feature-edit-button" data-edit-player-attack="${index}">✎</button><h4>${escapeHtml(item.name || "Без названия")}</h4><div class="weapon-badges"><b>${escapeHtml(item.damageDie || "Без кости")}</b>${item.proficient ? "<b>Владение</b>" : ""}</div><div>${window.RichText.render(item.description || "", escapeHtml)}</div></article>`,
      )
      .join("") || "<p class='empty'>Добавьте первое оружие.</p>";
  ["personality", "ideals", "bonds", "flaws"].forEach((key) => {
    const container = $(`[data-player-trait-cards="${key}"]`);
    container.innerHTML =
      (traitCards[key] || [])
        .map((item, index) =>
          item._editing
            ? `<article class="feature-card-editor" data-player-trait="${key}:${index}"><div class="feature-card-title"><input data-field="name" value="${escapeHtml(item.name || "")}" placeholder="Название"><button type="button" class="danger" data-delete-player-trait="${key}:${index}">Удалить</button></div><textarea data-field="description" placeholder="Описание">${escapeHtml(item.description || "")}</textarea><button type="button" class="feature-save-button" data-save-player-trait="${key}:${index}">Сохранить</button></article>`
            : `<article class="feature-card-view"><button type="button" class="feature-edit-button" data-edit-player-trait="${key}:${index}">✎</button><h4>${escapeHtml(item.name || "Без названия")}</h4><div>${window.RichText.render(item.description || "", escapeHtml)}</div></article>`,
        )
        .join("") || "<p class='empty'>Карточек пока нет.</p>";
  });
...
function renderShops(shops = []) {
  $("#player-shops").innerHTML = shops.length
    ? shops
        .map(
          (shop) => `
    <details class="player-shop ${escapeHtml(shop.status || "open")}">
      ...
      <div class="player-shop-table">${(shop.items || []).map((item) => `<article class="${item.sold ? "sold" : ""}"><strong>${escapeHtml(item.name)}</strong><b>${escapeHtml(item.price)}</b><small>${escapeHtml(item.quantity)}</small><p>${escapeHtml(item.description)}</p></article>`).join("") || "<p class='empty'>Товары пока не выставлены.</p>"}</div>
        ...
        .join("")
    : "<p class='empty'>Сейчас доступных торговцев нет.</p>";
}
...
function renderDerivedStats() {
  ...
  const scores = Object.fromEntries(
    abilities.map(([key]) => [
      key,
      Math.max(1, Math.min(30, Number(form.elements[`ability-${key}`]?.value || 10))),
    ]),
  );
  abilities.forEach(([key]) => {
    ...
    if (save)
      save.textContent = signed(modifier(scores[key]) + (form.elements[`save-${key}`]?.checked ? prof : 0));
  });
  ...
  $("#player-passive").textContent =
    10 + modifier(scores.wis) + (perceptionExpertise ? prof * 2 : perception ? prof : 0);
...
function renderCombat(combat) {
  ...
  if (self) {
    ...
    const available = Math.max(
      0,
      Number(character.hitDiceTotal ?? character.level ?? 1) - Number(character.usedHitDice || 0),
    );
    $("#heal-with-hit-die").disabled = available <= 0 || self.hp >= self.maxHp;
  }
  $("#combat-round").textContent = `Раунд ${combatState.round}`;
  $("#combat-turn-title").textContent = current?.isSelf
    ? "Сейчас твой ход"
    : `Ходит: ${current?.name || "—"}`;
  $("#combat-order").innerHTML = combatState.participants
    .map((item) => {
  ...
  if (self) {
    $("#player-death-saves strong").textContent =
      `Спасброски: ${"●".repeat(self.deathSuccesses || 0)}${"○".repeat(3 - (self.deathSuccesses || 0))} успехи · ${"●".repeat(self.deathFailures || 0)}${"○".repeat(3 - (self.deathFailures || 0))} провалы`;
    $$("#player-death-saves [data-death-save]").forEach(
      (button) => (button.disabled = !self.canMarkDeathSave),
    );
  }
}
...
async function loadCombat() {
  try {
    renderCombat(await api("/api/player/combat"));
  } catch {}
}
...
async function receivePlayerUpdate() {
  ...
  try {
    ...
    setStatus("Получены изменения мастера");
  } catch {}
}
...
function render(data) {
  ...
  featureCards = structuredClone(
    character.featureCards ||
      (character.features ? [{ name: "Общие умения и особенности", description: character.features }] : []),
  ).map((feature) => ({ ...feature, _editing: false }));
  attackCards = structuredClone(
    character.attackCards ||
      (character.attacks
        ? [{ name: "Общие атаки", damageDie: "", proficient: false, description: character.attacks }]
        : []),
  ).map((item) => ({ ...item, _editing: false }));
  traitCards = Object.fromEntries(
    ["personality", "ideals", "bonds", "flaws"].map((key) => [
      key,
      structuredClone(
        character.traitCards?.[key] ||
          (character[key] ? [{ name: "Общее", description: character[key] }] : []),
      ).map((item) => ({ ...item, _editing: false })),
    ]),
  );
  ...
  $("#abilities").innerHTML = abilities
    ...
    .join("");
  $("#player-saves").innerHTML = abilities
    .map(
      ([key, label]) =>
        `<label><input name="save-${key}" type="checkbox" ${(character.saveProficiencies || []).includes(key) ? "checked" : ""}><b data-player-save-value="${key}">+0</b><span>${label}</span></label>`,
    )
    .join("");
  $("#player-skills").innerHTML = abilities
    .map(
      ([ability, label]) =>
        `<section class="sheet-skill-group"><h5>${label}</h5>${skills
          .filter((item) => item[2] === ability)
          .map(
            ([key, skill]) =>
              `<label><input name="skill-${key}" type="checkbox" ${(character.skillProficiencies || []).includes(key) ? "checked" : ""}><b data-player-skill-value="${key}">+0</b><span>${skill}</span><input class="expertise-check" name="expertise-${key}" type="checkbox" ${(character.skillExpertise || []).includes(key) ? "checked" : ""} title="Экспертиза"></label>`,
          )
          .join("")}</section>`,
    )
    .join("");
  ...
  $("#hit-dice-status").textContent =
    `${Math.max(0, hitDiceTotal - usedHitDice)} из ${hitDiceTotal} костей d${Number(character.hitDieType || 8)} доступно`;
  $("#heal-with-hit-die").disabled =
    usedHitDice >= hitDiceTotal || Number(character.hp || 0) >= Number(character.maxHp || 0);
  ...
  knownNpcs = structuredClone(data.npcs || []);
  $("#known-npcs").innerHTML = knownNpcs.length
    ? knownNpcs
        .map(
          (npc, index) =>
            `<article class="known-npc-card" data-open-player-npc="${index}" tabindex="0" role="button">${npc.portraitUrl ? `<img src="${escapeHtml(npc.portraitUrl)}" alt="${escapeHtml(npc.name)}">` : `<span class="known-npc-silhouette">?</span>`}<div><span>${escapeHtml(npc.nickname || npc.race || "Известная личность")}</span><h4>${escapeHtml(npc.name)}</h4><p>${escapeHtml(npc.character || npc.appearance || "Мастер пока открыл только часть сведений.")}</p><small>Открыто сведений: ${npc.revealedFields?.length || 0}</small></div><b>Открыть досье →</b></article>`,
        )
        .join("")
    : "<p class='empty'>Твоему персонажу пока не открыли ни одного NPC.</p>";
  $("#public-lore").innerHTML = knownLore.length
    ? knownLore
        .map(
          (item, index) =>
            `<article class="lore-card openable" data-open-player-lore="${index}" tabindex="0" role="button">${item.images?.length ? `<div class="player-lore-images"><img src="${escapeHtml(item.images[0].url)}" alt="${escapeHtml(item.images[0].title || item.title)}">${item.images.length > 1 ? `<b>+${item.images.length - 1}</b>` : ""}</div>` : ""}<span>${escapeHtml(item.type)}</span><h3>${escapeHtml(item.title)}</h3>${item.ideology ? `<strong>${window.RichText.render(item.ideology, escapeHtml)}</strong>` : ""}<p>${window.RichText.render(item.text, escapeHtml)}</p><button type="button" class="lore-read-more" data-open-player-lore="${index}">Открыть досье</button></article>`,
        )
        .join("")
    : "<p class='empty'>Мастер пока ничего не опубликовал.</p>";
  renderSpells();
  renderInventory();
  renderFeatures();
...
async function saveCharacter() {
  const form = $("#sheet-page");
  const update = {};
  ["name", "className", "race", "background", "alignment", "equipment", "proficiencies"].forEach(
    (key) => (update[key] = form.elements[key].value),
  );
  ["level", "xp", "ac", "speed", "gold", "cp", "sp", "ep", "pp"].forEach(
    (key) => (update[key] = Number(form.elements[key].value || 0)),
  );
  update.inspiration = form.elements.inspiration.checked;
  update.abilities = Object.fromEntries(
    abilities.map(([key]) => [
      key,
      Math.max(1, Math.min(30, Number(form.elements[`ability-${key}`].value || 10))),
    ]),
  );
  update.saveProficiencies = abilities
    .filter(([key]) => form.elements[`save-${key}`].checked)
    .map(([key]) => key);
  update.skillProficiencies = skills
    .filter(([key]) => form.elements[`skill-${key}`].checked)
    .map(([key]) => key);
  update.skillExpertise = skills
    .filter(([key]) => form.elements[`expertise-${key}`].checked)
    .map(([key]) => key);
  ...
  update.attackCards = attackCards.map(({ name, damageDie, proficient, description }) => ({
    name,
    damageDie,
    proficient,
    description,
  }));
  update.traitCards = Object.fromEntries(
    Object.entries(traitCards).map(([key, items]) => [
      key,
      items.map(({ name, description }) => ({ name, description })),
    ]),
  );
  update.spellSlots = Array.from({ length: 9 }, (_, index) => ({
    level: index + 1,
    max: Number($(`[data-slot-max="${index + 1}"]`).value || 0),
    used: Number($(`[data-slot-used="${index + 1}"]`).value || 0),
  }));
  setStatus("Сохраняем...");
...
if (inviteToken) {
  ...
  $("#login-view p").textContent =
    "Придумайте логин и пароль. После регистрации вы получите собственный новый лист персонажа и сможете заполнить его самостоятельно.";
}
$("#show-login").onclick = () => {
...
function openPlayerNpc(index) {
  const npc = knownNpcs[index];
  if (!npc) return;
  const status = { alive: "Жив", dead: "Мёртв", missing: "Пропал", unknown: "Неизвестно" }[npc.status];
  const attitude = {
    "-2": "Враждебный",
    "-1": "Неприязненный",
    0: "Нейтральный",
  ...
  const fact = (label, value) => (value ? `<span><b>${label}</b>${escapeHtml(value)}</span>` : "");
  const section = (title, value) =>
    value
      ? `<section><h3>${title}</h3><div>${window.RichText.render(value, escapeHtml)}</div></section>`
      : "";
  $("#player-npc-detail").innerHTML =
    `<header class="player-npc-head">${npc.portraitUrl ? `<img src="${escapeHtml(npc.portraitUrl)}" alt="${escapeHtml(npc.name)}">` : `<span class="player-npc-silhouette">?</span>`}<div><span>Известный персонаж</span><h2>${escapeHtml(npc.name)}</h2><p>${escapeHtml(npc.nickname || npc.race || "")}</p></div></header><div class="player-npc-facts">${fact("Статус", status)}${fact("Раса", npc.race)}${fact("Местонахождение", npc.location)}${fact("Фракция", npc.faction)}${fact("Отношение к партии", attitude)}</div><div class="player-npc-sections">${section("Внешность", npc.appearance)}${section("Характер и манера речи", npc.character)}${section("Известные связи", npc.connections)}</div>`;
  window.RichText.enhance($("#player-npc-detail"));
  $("#player-npc-dialog").showModal();
}
$("#known-npcs").addEventListener("click", (event) => {
  const target = event.target.closest("[data-open-player-npc]");
  if (target) openPlayerNpc(Number(target.dataset.openPlayerNpc));
});
$("#known-npcs").addEventListener("keydown", (event) => {
  if (!["Enter", " "].includes(event.key)) return;
  const target = event.target.closest("[data-open-player-npc]");
  if (target) {
    event.preventDefault();
    openPlayerNpc(Number(target.dataset.openPlayerNpc));
  }
});
$("#close-player-npc").onclick = () => $("#player-npc-dialog").close();
$("#player-npc-dialog").onclick = (event) => {
  if (event.target === $("#player-npc-dialog")) $("#player-npc-dialog").close();
};
...
$("#public-lore").addEventListener("keydown", (event) => {
  ...
  if (target) {
    event.preventDefault();
    openPlayerLore(Number(target.dataset.openPlayerLore));
  }
});
$("#close-player-lore").onclick = () => $("#player-lore-dialog").close();
$("#player-lore-dialog").onclick = (event) => {
  if (event.target === $("#player-lore-dialog")) $("#player-lore-dialog").close();
};
$("#save-character").onclick = saveCharacter;
$("#save-notes").onclick = async () => {
  const body = JSON.stringify({
    version: 2,
    knowledge: $("#journal-knowledge").value,
    memories: $("#journal-memories").value,
    goals: $("#journal-goals").value,
    notes: $("#personal-notes").value,
  });
  ...
  (button) =>
    (button.onclick = () => {
      ...
      $$(".tab-page").forEach((page) =>
        page.classList.toggle("active", page.id === `${button.dataset.tab}-page`),
      );
    }),
);
...
$("#add-player-attack").onclick = () => {
  attackCards.push({ name: "", damageDie: "", proficient: false, description: "", _editing: true });
  markDirty();
  renderCharacterCards();
};
$$("[data-add-player-trait]").forEach(
  (button) =>
    (button.onclick = () => {
      traitCards[button.dataset.addPlayerTrait].push({ name: "", description: "", _editing: true });
      markDirty();
      renderCharacterCards();
    }),
);
$("#player-spells").addEventListener("input", (event) => {
  ...
  spells[+card.dataset.spell][event.target.dataset.field] =
...
$("#player-inventory").addEventListener("input", (event) => {
  ...
  if (!card || !event.target.dataset.field) return;
  inventoryItems[+card.dataset.inventoryItem][event.target.dataset.field] =
    event.target.type === "checkbox"
      ? event.target.checked
      : event.target.type === "number"
        ? Number(event.target.value)
...
$("#player-attack-cards").addEventListener("input", (event) => {
  const card = event.target.closest("[data-player-attack]"),
    field = event.target.dataset.field;
  if (card && field) {
    attackCards[+card.dataset.playerAttack][field] =
      event.target.type === "checkbox" ? event.target.checked : event.target.value;
    markDirty();
  }
});
$("#player-attack-cards").addEventListener("click", (event) => {
  for (const action of ["edit", "save", "delete"]) {
    const value = event.target.dataset[`${action}PlayerAttack`];
    if (value === undefined) continue;
    if (action === "delete") attackCards.splice(+value, 1);
    else attackCards[+value]._editing = action === "edit";
    markDirty();
    renderCharacterCards();
    break;
  }
...
$$("[data-player-trait-cards]").forEach((container) => {
  container.addEventListener("input", (event) => {
    const card = event.target.closest("[data-player-trait]"),
      field = event.target.dataset.field;
    if (card && field) {
      const [key, index] = card.dataset.playerTrait.split(":");
      traitCards[key][+index][field] = event.target.value;
      markDirty();
    }
  });
  container.addEventListener("click", (event) => {
    for (const action of ["edit", "save", "delete"]) {
      const value = event.target.dataset[`${action}PlayerTrait`];
      if (value === undefined) continue;
      const [key, index] = value.split(":");
      if (action === "delete") traitCards[key].splice(+index, 1);
      else traitCards[key][+index]._editing = action === "edit";
      markDirty();
      renderCharacterCards();
      break;
    }
...
$("#sheet-page").addEventListener("input", (event) => {
  ...
  $("#character-subtitle").textContent =
    [form.elements.race?.value, form.elements.className?.value, form.elements.background?.value]
      .filter(Boolean)
      .join(" · ") || "История героя продолжается";
  markDirty();
});
...
$("#heal-with-hit-die").onclick = async () => {
  try {
    ...
    $("#hit-dice-status").textContent =
      `${available} из ${total} костей d${character.hitDieType || 8} доступно`;
    $("#heal-with-hit-die").disabled = available <= 0 || Number(character.hp) >= Number(character.maxHp);
    $("#healing-result").textContent =
      `d${result.die}: ${result.roll}${result.constitution ? ` ${signed(result.constitution)}` : ""} = лечение ${result.healing}`;
...
$("#player-death-saves").addEventListener("click", async (event) => {
  ...
  try {
    renderCombat(
      await api("/api/player/combat/death-save", { method: "POST", body: JSON.stringify({ result }) }),
    );
  } catch (error) {
    setStatus(error.message);
  }
