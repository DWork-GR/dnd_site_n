Object.assign(DndApp, {
  ...
  characterCard(c) {
    ...
    const portrait = c.portraitUrl ? `<img class="portrait portrait-image" src="${this.escapeHtml(c.portraitUrl)}" alt="">` : `<span class="portrait">${this.initials(c.name)}</span>`;
    if (c.kind === "npc") {
      const status = { alive: "Жив", dead: "Мёртв", missing: "Пропал", unknown: "Неизвестно" }[c.npc?.status] || "Статус не указан";
      const location = this.data.lore.find(item => item.id === c.npc?.locationId)?.title || "Место не указано";
      const faction = this.data.lore.find(item => item.id === c.npc?.factionId)?.title || "Без фракции";
      return `<article class="character-card npc-card openable" data-open-npc="${index}"><div class="character-head">${portrait}<div><h3>${this.escapeHtml(c.name)}</h3><p>${this.escapeHtml(c.npc?.nickname || c.race || "NPC")} · ${this.escapeHtml(status)}</p></div></div><div class="npc-card-details"><span>${this.escapeHtml(location)}</span><span>${this.escapeHtml(faction)}</span></div><div class="character-foot"><span>${this.escapeHtml(c.npc?.character || "Характер пока не описан")}</span><button class="text-button" data-open-npc="${index}">Открыть досье →</button></div></article>`;
    }
  ...
  renderOverview() {
    ...
    const hp = players.length ? players.reduce((sum, c) => sum + (c.maxHp ? c.hp / c.maxHp : 0), 0) / players.length * 100 : 0;
    this.$("#health-average").textContent = `${Math.round(hp)}%`;
    this.$("#gold-total").textContent = players.reduce((sum, c) => sum + Number(c.gold || 0), 0).toLocaleString("ru");
    this.$("#overview-party").innerHTML = players.slice(0, 4).map(c => `<div class="party-row"><span class="portrait">${this.initials(c.name)}</span><div><strong>${this.escapeHtml(c.name)}</strong><small>${this.escapeHtml(c.className || "")} · ${c.level} уровень</small></div><div><div class="hpbar"><i style="width:${c.maxHp ? c.hp / c.maxHp * 100 : 0}%"></i></div><div class="hptext">${c.hp} / ${c.maxHp} здоровья</div></div><small>${this.escapeHtml(this.data.packs.find(p => p.id === c.packId)?.name || "")}</small></div>`).join("");
    this.$("#task-counter").textContent = `${this.data.tasks.filter(t => t.done).length} / ${this.data.tasks.length}`;
    this.$("#task-list").innerHTML = this.data.tasks.map((task, i) => `<label class="task ${task.done ? "done" : ""}"><input type="checkbox" data-task="${i}" ${task.done ? "checked" : ""}>${this.escapeHtml(task.text)}</label>`).join("");
  },

  renderCharacters(group = this.selectedPlayerGroup) {
    this.selectedPlayerGroup = group;
    const packs = this.data.packs.filter(pack => this.data.characters.some(c => c.kind === "player" && c.packId === pack.id));
  ...
  renderLore(type = "all") {
    ...
    this.$("#lore-list").innerHTML = list.map(entry => {
  ...
  renderCalendar() {
    ...
    this.$("#calendar-month").textContent = cursor.toLocaleDateString("ru-RU", { month: "long", year: "numeric" });
    ...
    this.$("#session-list").innerHTML = sorted.map(session => `<button class="session-entry ${session.status}" data-open-session="${session.id}"><time>${session.date ? new Date(`${session.date}T12:00:00`).toLocaleDateString("ru-RU", { day: "numeric", month: "long", year: "numeric" }) : "Без даты"}${session.time ? ` · ${session.time}` : ""}</time><strong>${this.escapeHtml(session.title)}</strong><small>${session.status === "completed" ? "Сыграна" : session.status === "cancelled" ? "Отменена" : "Запланирована"}${session.summary ? " · есть краткое содержание" : ""}</small></button>`).join("") || `<div class="empty-state"><strong>Сессий пока нет</strong><span>Запланируйте первую игру.</span></div>`;
  },
  ...
  async renderMedia() {
    try {
      ...
      const labels = { portrait: "Портрет", artifact: "Артефакт", note: "Записка", map: "Карта", other: "Прочее" };
      this.$("#media-gallery").innerHTML = this.mediaAssets.map(asset => `<article class="media-card"><button class="media-open" data-open-image="${this.escapeHtml(asset.url)}"><img src="${this.escapeHtml(asset.url)}" alt="${this.escapeHtml(asset.title)}"></button><div><span>${labels[asset.category] || asset.category}</span><strong>${this.escapeHtml(asset.title || asset.original_name)}</strong><small>${Math.round(asset.size_bytes / 1024)} КБ</small><button class="delete" data-delete-image="${asset.id}">Удалить</button></div></article>`).join("") || `<div class="empty-state"><strong>Медиатека пуста</strong><span>Загрузите первый портрет, артефакт или записку.</span></div>`;
    } catch {}
  },

  renderBestiary() {
    const type=this.$("#monster-filter-type")?.value||"all",cr=this.$("#monster-filter-cr")?.value||"all",tag=this.$("#monster-filter-tag")?.value||"all";
    const types=[...new Set(this.data.bestiary.map(x=>x.type).filter(Boolean))].sort(),crs=[...new Set(this.data.bestiary.map(x=>String(x.cr)).filter(Boolean))].sort((a,b)=>Number(a)-Number(b)),tags=[...new Set(this.data.bestiary.map(x=>x.campaignTag).filter(Boolean))].sort();
    this.$("#monster-filter-type").innerHTML=`<option value="all">Все типы</option>`+types.map(x=>`<option ${x===type?"selected":""}>${this.escapeHtml(x)}</option>`).join("");this.$("#monster-filter-cr").innerHTML=`<option value="all">Любая CR</option>`+crs.map(x=>`<option ${x===cr?"selected":""}>${this.escapeHtml(x)}</option>`).join("");this.$("#monster-filter-tag").innerHTML=`<option value="all">Все арки</option>`+tags.map(x=>`<option ${x===tag?"selected":""}>${this.escapeHtml(x)}</option>`).join("");
    const list=this.data.bestiary.filter(x=>(type==="all"||x.type===type)&&(cr==="all"||String(x.cr)===cr)&&(tag==="all"||x.campaignTag===tag));
    this.$("#bestiary-grid").innerHTML=list.map(m=>`<details class="monster-card"><summary><div><span>${this.escapeHtml(m.type)} · ${this.escapeHtml(m.size)}${m.campaignTag?` · ${this.escapeHtml(m.campaignTag)}`:""}</span><h3>${this.escapeHtml(m.name)}</h3><small>${this.escapeHtml(m.alignment||"Без мировоззрения")} · CR ${this.escapeHtml(m.cr)} · ${Number(m.xp||0).toLocaleString("ru")} XP</small></div><div><b>КД ${m.ac}</b><b>${m.hp} HP</b></div></summary><div class="monster-statblock"><button class="text-button monster-edit" data-open-monster="${m.id}">Редактировать</button><p><strong>Скорость</strong> ${this.escapeHtml(m.speed||"—")} · <strong>Кость хитов</strong> ${this.escapeHtml(m.hitDice||"—")}</p><div class="monster-ability-row">${this.abilities.map(([k,l])=>`<span><b>${l.slice(0,3).toUpperCase()}</b>${m.abilities[k]} (${this.signed(this.modifier(m.abilities[k]))})</span>`).join("")}</div>${m.advantageSaves?.length?`<p><strong>Спасброски с преимуществом</strong> ${m.advantageSaves.map(k=>this.abilities.find(([key])=>key===k)?.[1]||k).join(", ")}</p>`:""}${[["Навыки",m.skills],["Иммунитеты",m.damageImmunities],["Сопротивления",m.damageResistances],["Уязвимости",m.damageVulnerabilities],["Иммунитеты к состояниям",m.conditionImmunities],["Чувства",m.senses],["Пассивное восприятие",m.passivePerception],["Языки",m.languages]].filter(([,v])=>v).map(([l,v])=>`<p><strong>${l}</strong> ${this.escapeHtml(v)}</p>`).join("")}${m.traits?`<section><h4>Особые способности</h4><p>${this.escapeHtml(m.traits)}</p></section>`:""}${m.actions?.length?`<section><h4>Действия</h4>${m.actions.map(a=>`<p><strong>${this.escapeHtml(a.name)}.</strong> ${this.escapeHtml(a.description)}</p>`).join("")}</section>`:""}${[["Реакции",m.reactions],["Легендарные действия",m.legendaryActions],["Логово",m.lair],["Заметки мастера",m.dmNotes]].filter(([,v])=>v).map(([l,v])=>`<section><h4>${l}</h4><p>${this.escapeHtml(v)}</p></section>`).join("")}</div></details>`).join("")||`<div class="empty-state"><strong>Бестиарий пуст</strong><span>Добавьте первое существо.</span></div>`;
  },
  renderShops(){
    this.$("#shop-list").innerHTML=this.data.shops.map(s=>{const location=this.data.lore.find(x=>x.id===s.locationId);return `<details class="shop-card ${s.status}"><summary><div><span>${s.status==="open"?"Открыт":s.status==="closed"?"Закрыт":"Недоступен"}</span><h3>${this.escapeHtml(s.name)}</h3><small>${this.escapeHtml(location?.title||"Без локации")} · ${s.items.filter(x=>!x.sold).length} товаров в продаже</small></div><button class="text-button" data-open-shop="${s.id}">Редактировать</button></summary><p>${this.escapeHtml(s.description)}</p><div class="shop-table"><div class="shop-row head"><b>Товар</b><b>Цена</b><b>Наличие</b><b>Свойства</b></div>${s.items.map(i=>`<div class="shop-row ${i.sold?"sold":""}"><strong>${this.escapeHtml(i.name)}</strong><span>${this.escapeHtml(i.price)}</span><span>${this.escapeHtml(i.quantity)}</span><span>${this.escapeHtml(i.description)}</span></div>`).join("")||"<small>Ассортимент пуст.</small>"}</div></details>`}).join("")||`<div class="empty-state"><strong>Торговцев пока нет</strong><span>Добавьте первый магазин.</span></div>`;
  },
  ...
  renderInitiative() {
    ...
    this.$("#initiative-list").innerHTML = combat.active
      ? combat.participants.map((item, i) => `<div class="combat-master-row ${i === combat.turnIndex ? "active" : ""}">
          <b>${i + 1}</b>
      ...
      : this.data.initiative.map((item, i) => `<div class="initiative-row"><b>${i + 1}</b><span>${this.escapeHtml(item.name)}</span><input type="number" value="${item.value}" data-init="${i}"><button class="delete" data-delete-init="${i}">×</button></div>`).join("");
  ...
  renderWorkshop() {
    const d = this.data;
    this.$("#pack-list").innerHTML = `<button class="collection-link ${this.selectedPackId === "all" ? "active" : ""}" data-pack-id="all"><span>Все персонажи</span><b>${d.characters.length}</b></button>` + d.packs.map(pack => `<button class="collection-link ${this.selectedPackId === pack.id ? "active" : ""}" data-pack-id="${pack.id}"><span>${this.escapeHtml(pack.name)}<small>${this.escapeHtml(pack.type)}</small></span><b>${d.characters.filter(c => c.packId === pack.id).length}</b></button>`).join("");
    ...
    const roster = this.selectedPackId === "all" ? d.characters : d.characters.filter(c => c.packId === this.selectedPackId);
    this.$("#master-roster-grid").innerHTML = roster.map(c => this.characterCard(c)).join("") || `<div class="empty-state"><strong>Пачка пока пуста</strong><span>Добавьте первого персонажа или NPC.</span></div>`;
    this.$("#note-section-list").innerHTML = `<button class="collection-link ${this.selectedNoteSectionId === "all" ? "active" : ""}" data-note-section="all"><span>Все записи</span><b>${d.masterNotes.length}</b></button>` + d.noteSections.map(section => `<button class="collection-link ${this.selectedNoteSectionId === section.id ? "active" : ""}" data-note-section="${section.id}"><span>${this.escapeHtml(section.name)}</span><b>${d.masterNotes.filter(n => n.sectionId === section.id).length}</b></button>`).join("");
    this.$("#active-note-section").textContent = d.noteSections.find(s => s.id === this.selectedNoteSectionId)?.name || "Все записи";
    this.$("#rename-note-section").style.visibility = this.selectedNoteSectionId === "all" ? "hidden" : "visible";
    const notes = this.selectedNoteSectionId === "all" ? d.masterNotes : d.masterNotes.filter(n => n.sectionId === this.selectedNoteSectionId);
    if (!notes.some(n => n.id === this.selectedNoteId)) this.selectedNoteId = notes[0]?.id || null;
    this.$("#master-notes").innerHTML = notes.map(note => `<button class="note-link ${note.id === this.selectedNoteId ? "active" : ""}" data-note-id="${note.id}"><strong>${this.escapeHtml(note.title || "Без названия")}</strong><small>${this.escapeHtml(note.tags.join(" · ") || "без тегов")}</small></button>`).join("") || "<small>В этом разделе пока нет записей.</small>";
    const note = d.masterNotes.find(item => item.id === this.selectedNoteId);
    this.$("#note-title").value = note?.title || ""; this.$("#note-tags").value = note?.tags?.join(", ") || ""; this.$("#note-body").value = note?.body || "";
