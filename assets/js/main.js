(async () => {
  ...
  const loginMaster = async () => {
    ...
    return new Promise(resolve => {
      form.onsubmit = async event => {
        ...
        const response = await fetch("/api/master/login", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(Object.fromEntries(new FormData(form))) });
        const result = await response.json().catch(() => ({}));
        if (!response.ok) return document.querySelector("#master-login-error").textContent = result.error || "Не удалось войти";
        dialog.close();
        resolve(true);
      };
    });
  ...
  if (masterInvite) {
    ...
    await new Promise(resolve => {
      form.onsubmit = async event => {
        event.preventDefault();
        const response = await fetch("/api/master/register", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...Object.fromEntries(new FormData(form)), token: masterInvite }) });
        const result = await response.json().catch(() => ({}));
        if (!response.ok) return document.querySelector("#master-register-error").textContent = result.error || "Не удалось создать кабинет";
  ...
  try {
    ...
    if (response.ok) {
      ...
      await fetch("/api/state", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(A.data) });
  ...
  const loadMasterProfile = async () => {
    ...
    A.data.campaignSettings.campaignName = profile.campaign?.name || A.data.campaignSettings.campaignName || "Моя кампания";
  ...
  const updateMasterPreview = () => {
    ...
    const fallbacks = {
      displayName: "Хранитель", campaignName: "Моя кампания", campaignChapter: "Глава кампании",
      brandTitle: "Архив мастера", brandEyebrow: "Личная летопись", worldTitle: "Атлас мира",
      heroKicker: "Следующая игра", heroTitle: "Название приключения",
    ...
    preview.style.setProperty("--preview-accent", /^#[0-9a-f]{6}$/i.test(values.accentColor || "") ? values.accentColor : "#d7aa5e");
    preview.style.setProperty("--preview-image", values.heroImageUrl ? `url("${String(values.heroImageUrl).replaceAll('"', "%22")}")` : "radial-gradient(circle at 70% 25%,#8a65332b,transparent 35%)");
  };
  ...
  document.querySelector("#master-profile").onclick = async () => {
    ...
    ["brandTitle","brandEyebrow","worldTitle","campaignChapter","greeting","campaignDescription","heroTitle","heroKicker","heroDescription","heroImageUrl"].forEach(key => form.elements[key].value = settings[key] || "");
    form.elements.accentColor.value = /^#[0-9a-f]{6}$/i.test(settings.accentColor || "") ? settings.accentColor : "#d7aa5e";
  ...
  document.querySelector("#master-account-form").onsubmit = async event => {
    ...
    await fetch("/api/master/me", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ displayName: values.displayName, campaignName: values.campaignName }) });
  ...
  document.querySelector("#create-master-invite").onclick = async () => {
    const response = await fetch("/api/master/invitations", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ displayName: document.querySelector("#master-invite-name").value }) });
    const result = await response.json();
